...
class Driver extends Wrapper {
  ...
  static waitResultConverter(val, obj) {
    if (Array.isArray(val)) {
      return val.map(el => new Element(el, obj))
    }
    return new Element(val, obj)
  }
  ...
  constructor(_driver) {
    ...
    this.wait = Wrapper.wrapAsync(_driver, 'wait', Driver.waitResultConverter)
...
const ELEMENT_METHODS = `catch,clear,click,getAttribute,getCssValue,getId,
getRect,getTagName,getText,isDisplayed,isEnabled,isSelected,sendKeys,
...
class Element extends Wrapper {
  constructor(_ele, _driver) {
    ...
    this.delegate = () => _ele
    this.findElement = Wrapper.wrapAsync(_ele, 'findElement',
      (val, obj) => new Element(val, obj))
  }

  findElements(by) {
    return Wrapper.syncify(this.ele.findElements(by)).map(el => new Element(el, this.driver))
  }
}
